remove_mock = function(physeq = decontam_physeq,
  ...
  phyloseq_file <- list.files(destination_folder, pattern = "phyloseq_asv_level_without_mock\\.rds$", full.names = TRUE)
  ...
  new_file_path <- file.path(new_folder, paste0(projects, "_phyloseq_asv_level_without_mock.rds"))
